...
from database_manager import (
    insert_or_ignore_language,
    create_new_request,
    insert_sentence,
    link_vocab_to_request,
)
...
{{
    ...
    "type": "generated"
        ...
        # TODO: create guardrails/system to re-ask the prompt when LLM doesn't answer properly
    ...
    def pick_random_terms_from_vocab(self) -> pd.DataFrame:
        ...
        random_nouns = (
            df[df["type"] == "noun"].sample(n=randrange(max_nb_noun // 2, max_nb_noun))
            if max_nb_noun > 0
            else pd.DataFrame()
        )

        random_verbs = (
            df[df["type"] == "verb"].sample(n=randrange(max_nb_verb // 2, max_nb_verb))
            if max_nb_verb > 0
            else pd.DataFrame()
        )

        random_adjs = (
            df[df["type"] == "adjective"].sample(
        ...
        random_advs = (
            df[df["type"] == "adverb"].sample(n=randrange(max_nb_adv // 2, max_nb_adv))
            if max_nb_adv > 0
            else pd.DataFrame()
        )
